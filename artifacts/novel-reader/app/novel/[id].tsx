...
export default function ReaderScreen() {
  ...
  // ─ Chapter content loader — guarded against stale/out-of-order async results ─
  // Rapid chapter navigation can fire several of these before earlier ones
  // resolve. The `cancelled` flag ensures only the most recent request for the
  // current chapterIndex is allowed to commit state.
  useEffect(() => {
    let cancelled = false;
  ...
  // Combined index+content snapshot, updated atomically in a single effect so
  // persistChapterContent can never observe an index paired with a stale
  // chapter's content (which previously could corrupt the wrong chapter file).
  ...
  // ─ Save chapter content to disk (used by both Option A and B below) ─
  // Defined before the effects that reference it since it's a `const`.
  ...
  const goChapter = (dir: 1 | -1) => {
    ...
    setReadingProgress(0);
    // Note: the optimistic "apply preloaded next-chapter content immediately"
    // shortcut that used to live here was removed. The guarded loadContent
    // effect above already re-derives paragraphs/sentences correctly from
    // the chapterCache (a fast cache hit when preloaded), so this block was
    // redundant and could momentarily desync chapterContent from
    // processedParagraphs/ttsSentences.
  };
          ...
          <Pressable style={styles.overlayDismiss} onPress={() => setShowSettingsSheet(false)}>
            ...
            >
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 16 }}>
                ...
                <View style={styles.rowGroup}>
                  <Text style={[styles.rowGroupLabel, { color: adaptiveColors.textSecondary }]}>AUTO SCROLL</Text>
                ...
                {/* 4. MARGINS */}
                <Text style={[styles.sectionLabel, { color: adaptiveColors.textSecondary, marginTop: 4 }]}>MARGINS</Text>
                    ...
                    <Pressable
                      ...
                      style={[
                        styles.marginPresetBtn,
                        {
                          ...
                          borderColor: marginPresetIdx === idx ? adaptiveColors.accent : adaptiveColors.border,
                        },
                      ]}
                      onPress={() => {
