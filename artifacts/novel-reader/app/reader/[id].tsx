...
export default function ReaderScreen() {
  ...
  const [quickActionsExpanded, setQuickActionsExpanded] = useState(true); // <-- added
  ...
  // ── Navigation (depends on stopTTS and cancelAutoNext) ──
  const {
    goChapter,
    handleChapterSelect,
    continueReading,
            ...
            <Pressable
              ...
              onPress={() => {
                if (ttsIndex >= 0 && ttsIndex < ttsSentences.length) {
                  toggleTTS(); // This will resume from the current index if stalled
                } else {
                  stopTTS();
                }
              }}
