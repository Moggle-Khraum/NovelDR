...
export function useTTS({
  ...
  // Bumped every time speech is deliberately (re)started from outside the
  // normal onDone chain - fresh play, stall-resume, or the background/
  // foreground resync. Any in-flight callback from a chain that was
  // speaking when one of those happens isn't guaranteed to be cancelled
  // just because Speech.stop() was called - if it fires anyway, checking
  // this first turns it into a no-op instead of letting it drag the
  // highlight back to a stale index in a race with the fresh restart.
      ...
      const advance = () => {
        ...
        if (isLast) {
          ...
          setTimeout(() => {
            if (!isMountedRef.current || !ttsActiveRef.current) return;
            if (ttsGenerationRef.current !== gen) return;
            speakChunks(chunks, chunkIdx + 1, onAllDone, onChunkError, gen);
          }, TTS_PAUSE_MS);
        }
      };
      if (!chunk) {
  ...
  const speakSentence = useCallback(
    (sentences: string[], index: number) => {
      ...
      // Captured once per speakSentence call. A naturally-chained call
      // (onSentenceDone -> speakSentence(next)) re-reads the still-current
      // value, so it inherits the same generation as the call before it -
      // only an external restart (toggleTTS, the AppState resync, or
      // stopTTS) actually changes it. Everything below that can fire
      // later and asynchronously checks against this snapshot.
