...
export const novelPhoenixScraper: SourceScraper = {
  ...
  fetchNovelMeta: async (url: string): Promise<NovelMeta> => {
    ...
    const coverUrl =
      safeMatch(html, /<div[^>]*class="glass-background"[^>]*>\s*<img[^>]*src="([^"]+)"/i) ?? '';

    // <h1 class="novel-title ...">Title</h1>
    const title = decodeEntities(
      safeMatch(html, /<h1[^>]*class="novel-title[^"]*"[^>]*>([^<]+)<\/h1>/i) ?? 'Unknown Title',
    );
    ...
    const contentBlock = extractByDepth(summaryBlock, 'class="content') ?? summaryBlock;
    const synopsis = extractParagraphs(contentBlock);

    // <a ... href="/novel/{slug}/chapter-1" ...>Read Now</a>
    const firstChapterPath = safeMatch(html, /<a[^>]*href="([^"]+)"[^>]*>Read Now<\/a>/i);
